...
const token = '7306041217:AAE1xcrGZsYFcr6xhDwh5WnDrAzjPA-ArMI';
const fs = require('fs');
...
function getBalance(apikey) {
    const index = fetch('https://api.sms-activate.io/stubs/handler_api.php?api_key=' + apikey + '&action=getBalance', {})
...
function getNumber(apikey, service, operator, country) {
    const index = fetch('https://api.sms-activate.io/stubs/handler_api.php?api_key=' + apikey + '&action=getNumber&service=' + service + '&operator=' + operator + '&country=' + country + '', {})
...
function getCode(apikey, orderid) {
    const index = fetch('https://api.sms-activate.io/stubs/handler_api.php?api_key=' + apikey + '&action=getStatus&id=' + orderid + '', {})
...
function successorder(apikey, orderid) {
    const index = fetch('https://api.sms-activate.io/stubs/handler_api.php?api_key=' + apikey + '&action=setStatus&status=6&id=' + orderid + '', {})
...
function closeOrder(apikey, orderid) {
    const index = fetch('https://api.sms-activate.io/stubs/handler_api.php?api_key=' + apikey + '&action=setStatus&status=8&id=' + orderid + '', {})
...
function resendOrder(apikey, orderid) {
    const index = fetch('https://api.sms-activate.io/stubs/handler_api.php?api_key=' + apikey + '&action=setStatus&status=3&id=' + orderid + '', {})
